...
reminderScene.enter(ctx =>{ctx.reply(
    'message format: "event dd.mm.yy hh.mm.ss"',
    exit_keyboard)    
})
...
reminderScene.on('text', async ctx=>{
    ...
    var year = arr_user_message[arr_user_message.length-2].split('.')   // get dd.mm.yy
    var time = arr_user_message[arr_user_message.length-1]              //get time

    var reminer_time = year.join('.') + ' ' + time    //year + time
    const date = new Date(`${year[1]}.${year[0]}.${year[2]} ${time}`)   //date in the format mm.dd.yy

    if(arr_user_message.length < 3 || !date_pattern.test(reminer_time) || date == 'Invalid Date') //check message format 
    {
